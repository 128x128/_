...
class Tensor():
	def __init__(self, shape=(1,1), data=None, dtype=None): 
		id         = None
		self.data  = data 
	...
	# initializers
	@staticmethod
	def fill(*shape):  return Tensor(shape, data=[None  for _ in range(math.prod(shape))], dtype=float)
	@staticmethod
	def ones(*shape):  return Tensor(shape, data=[1 for _ in range(math.prod(shape))], dtype=float)
	@staticmethod
	def zeros(*shape): return Tensor(shape, data=[0 for _ in range(math.prod(shape))], dtype=float)
	@staticmethod
	def randn(*shape): return Tensor(shape, data=[random.uniform(-1, 1) for _ in range(math.prod(shape))], dtype=float)
	@staticmethod
	def arange(*shape): return Tensor(shape, data=[_+1 for _ in range(math.prod(shape))], dtype=float)
	@staticmethod
	def op(*shape): return Tensor(shape, data=[Function() for _ in range(math.prod(shape))], dtype=Function)
	@staticmethod
	def str(*shape): return Tensor(shape, data=[" " for _ in range(math.prod(shape))], dtype=str)
	...
	def __str__(self): 
		if self.dtype==Char: return "".join([_.data for _ in self.data])
	...
	# BINARY OPS
	def add(x, y): return Function(x, y, op=_add).apply(Tensor.fill(*x.shape))
	def sub(x, y): return Function(x, y, op=_sub).apply(Tensor.fill(*x.shape))
	def mul(x, y): return Function(x, y, op=_cmul).apply(Tensor.fill(*x.mulshape(y)))
	...
	# REDUCE OPS
	def sum(self): return Function(self, op=self._sum).apply(Variable())
	def min(self): return Function(self, op=self._min).apply(Variable())
	def max(self): return Function(self, op=self._max).apply(Variable())
	def mean(self): return Function(self, op=self._mean).apply(Variable())
	def avg(self): return Function(self, op=self._avg).apply(Variable())
	...
	def pink(self, buffer:str): return f"\033[95m{buffer}\033[0m"
	
	def charbuff(self, n:int): return [" " for _ in range(n)]
	def replace_char(self, _:int, char:str): self.data[_].data=char 
	def inter_replace(self, n:int, token:str): 
		for _ in range(int(self.size/n)-1): self.replace_char((_+1)*n, token)
		return self
