...
class Variable: 
	...
	def add(a, b): return a._.data + b._.data 
...
class Node:
	...
	def apply(self, op, x): self.fn = Node.Function(op, x, self); return self
...
class Tensor: 
	...
	def _stride_(self): return tuple([math.prod(self.shape[_+1:len(self.shape)]) for _ in range(len(self.shape))])
	def reshape(self, shape): self.shape = shape; self.stride=self._stride_(); return self.shape
	...
	def __add__(a, b): return Node.new(id=f"({a._.id} + {b._.id})", type=Tensor(a.shape), data=[a[_]+b[_] for _ in range(len(a._.data))]).apply(Function.add, (a, b))
